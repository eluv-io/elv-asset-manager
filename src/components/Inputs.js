...
import {rootStore, formStore, contentStore} from "../stores";
import {
  Confirm,
  IconButton,
  TextArea,
...
const InfoField = observer(({
  ...
  try {
    ...
    const key = `input-${field.name}-${formStore.localizationKey}`;

    if(formStore.localizationKey && (field.no_localize || field.path)) {
      return null;
    }

    let fieldType = field.type;
    if(fieldType === "reference_type") {
      fieldType = (Utils.SafeTraverse(HEAD || {}, ...(ReferencePathElements(PATH, field.reference))) || "text");
    }

    if(field.depends_on) {
      const dependentValues = (Array.isArray(field.depends_on) ? field.depends_on : [field.depends_on])
        .map(depends_on => Utils.SafeTraverse(HEAD || {}, ...(ReferencePathElements(PATH, depends_on))))
        .filter(value => value);
    ...
    if(fieldType === "hidden") {
            ...
            <IconButton
              ...
              onClick={async () =>
                await Confirm({
                  ...
                  onConfirm: () => Update(field.name, rootStore.SelfEmbedUrl(field.version, field))
                })
              }
            />
          </div>
      ...
      if(fieldType === "reference_select") {
        options = (Utils.SafeTraverse(HEAD || {}, ...(ReferencePathElements(PATH, field.reference))) || [])
      ...
      let options = [
        ...(formStore.availableNTPs || []).map(({ntpId, name}) => [name, ntpId])
      ];

      const selected = (formStore.availableNTPs || []).find(({ntpId}) => ntpId === entry[field.name]);
      ...
      if(fieldType === "reference_list") {
        fields = field.fields || [];
        fields = fields.concat(Utils.SafeTraverse(HEAD || {}, ...(ReferencePathElements(PATH, field.reference))) || []);
      }
        ...
        <ObjectSelection
          ...
          Update={async () => {
            await Update(field.name, {
              ...entry[field.name],
              versionHash: await contentStore.LatestVersionHash(entry[field.name])
            });
          }}
...
const ListField = observer(({
  ...
  // Forcibly re-render when swapping to ensure content is changed
  const [renderKey, setRenderKey] = useState(0);
  try {
    if(formStore.localizationKey && field.no_localize) {
      return null;
    }
        ...
        return (
          ...
          >
            <div className="actions">
              {Maybe(orderable && !formStore.localizationKey, <OrderButtons index={index} length={values.length} Swap={Swap}/>)}
              {
                Maybe(
                  !formStore.localizationKey,
                  <IconButton
                    icon={DeleteIcon}
    ...
    return (
      <LabelledField
        key={`list-field-container-${name}-${renderKey}`}
        ...
        value={
          <div className={`list-field ${!fields ? "array-list" : ""}`}>
            {fieldInputs}
            {formStore.localizationKey ? null : addButton}
          </div>
        }
      />
    );
...
const RecursiveField = observer(({list, ...args}) =>
  list ?
    <ListField {...args} /> :
    <InfoField {...args} />
);
